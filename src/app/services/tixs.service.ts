...
export class TixsService {
 ...
 loadedProducts:boolean=false;
  ...
  public loadInfo2(){
  	this.http.get("https://192.168.0.107:1350/product")
    .subscribe(data =>{
  	  this.loadedProducts=true;
  	  this.AllLoadedProducts=data.json();
  	  this.products=this.AllLoadedProducts.results;   
      for (var i=0;i<this.products.length;i++){
        this.products[i].image="https://192.168.0.107/web/"+this.products[i].images[0];
        ...
        if(this.products[i].images.length<2){
          ...
          for (var j=0;j<this.products[i].images.length;j++){
            this.products[i].imagesG[j]="https://192.168.0.107/web/"+this.products[i].images[j];
          }
        }
      ...
      this.http.get("https://192.168.0.107:1350/category").subscribe(data =>{
