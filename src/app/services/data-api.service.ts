...
export class DataApiService {
 ...
 public card : CardInterface ={
	...
	getAllTixs(){
		const url_api = 'https://192.168.0.107:3000/api/product?filter[where][status]=activated';
	...
	getTixById(id:string){
		//console.log(id);
		let indice = id;
		const url_api=`https://192.168.0.107:3000/api/product/${indice}`;
		this.tix = this.http.get(url_api);
		return (this.tix);
	}
	getPending(){
		const url_api='https://192.168.0.107:3000/api/product?filter[where][status]=pending';
		return (this.tixs = this.http.get(url_api));
	}
	saveTix(tix :TixInterface){
		let token = this.authService.getToken();
		const url_api='https://192.168.0.107:3000/api/product?access_token${token}';
		return this.http
	...
	getCards(id:string){
		let indice = id;
		const url_api = "https://192.168.0.107:3000/api/card?filter[where][userd]=a"+indice;
		return this.http.get(url_api);
	}
	saveCard(card :CardInterface){
		let token = this.authService.getToken();
		const url_api='https://192.168.0.107:3000/api/card?access_token${token}';
		return this.http
