...
export class DataApiService {
  ...
  constructor(private http: HttpClient, private authService:AuthService) {}
	...
	getAllTixs(){
		const url_api = 'https://www.thetixsapp.com:3000/api/tixs'
		return this.http.get(url_api);

	}
	getTixById(id:string){
		const url_api='https://www.thetixsapp.com:3000/api/tixs/${id}';
		return (this.tix = this.http.get(url_api));
	}
	getOffers(){
		const url_api='https://www.thetixsapp.com:3000/api/tixs?filter[where][Offers]=1';
		return (this.tixs = this.http.get(url_api));
	}
	saveTix(tix :TixInterface){
		let token = this.authService.getToken();
		const url_api='https://www.thetixsapp.com:3000/api/tixs?access_token${token}';
		return this.http
